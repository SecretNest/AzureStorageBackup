...
| Checking catalog | catalog | the `PRAGMA quick_check` of the container's catalog file, a full read of it, owed only when the last process that wrote the catalog did not exit cleanly (the `.open` marker beside it, see [storage-format.md](storage-format.md)) or a reader saw damage — a cleanly closed catalog is not re-read. It used to run unannounced inside the first write open — minutes of "Loading versions 100%" on an 8 GB history, read as a hang — and now has a line of its own, between Loading versions and Diffing, naming the file and its size ("catalog.db (8.2 GB)"). The check runs the same stage at its start, every time (the check is where an operator asks on purpose; see [check-restore-repair.md](check-restore-repair.md), "The local catalog"). It carries **no figures**: quick_check reports no progress, so there is no total, no percentage, no in-flight item and no speed — a 0% and a 0 B/s that stand for minutes read as a hang, where the file's size reads as the read it is. The counts line says "one full read of the catalog file". Pause is greyed as during Loading versions (there is nothing to park in); Suspend and Stop interrupt the statement and end the run, and the check simply runs again next time |
