...
| Checking catalog | catalog | the once-per-process `PRAGMA quick_check` of the container's catalog file, a full read of it, paid on the first run after the server starts and not again. It used to run unannounced inside the first write open of the catalog — tens of seconds of "Loading versions 100%" on a big history, read as a hang — and now has a line of its own, between Loading versions and Diffing, showing the file it reads with its size ("catalog.db (1.8 GB)") so the wait reads as the read it is. One statement over one file, so the counts line says "one full read of the catalog file" rather than pretending to a fraction. Pause is greyed as during Loading versions (there is nothing to park in); Suspend and Stop interrupt the statement and end the run, and the check simply runs again next time |
