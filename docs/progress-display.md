...
| Upgrading catalog | versions | a version of a format-1 catalog converted in place to the current format, one transaction each, before anything else opens the file (see [storage-format.md](storage-format.md), "Converting a format-1 catalog"). It happens once per container, on the first backup after the upgrade that brought the format; every other write open converts silently, and this stage exists because a backup is where an operator is watching and the conversion is as long as the history is big. Counted like Loading versions: **versions** on the counts line, **entries** — the whole history's declared rows — on the done line, since that is what the time goes into. Both of the counts line's numbers come from the catalog rather than from the loop, because a conversion resumed after a Suspend converts only the versions left, and one told "10 versions" by the info file would finish at "6 of 10" beside "entries (100%)". The item line names the version under conversion, "version 4". Pause is greyed as during Loading versions (one transaction per version, nothing to park in); Suspend and Stop end the run between versions, and the conversion resumes at the next open from the first version not yet in |
| Loading versions | versions | a retained version made sure of in the catalog before the diff. Nothing on an ordinary run; on the first run after an upgrade it is the migration of the container's whole history, which can take a long time — and for as long as it had no stage of its own it sat under Scanning, where a migration taking hours read as a scan that had hung. Its completion and remaining time come from **entries** (index rows), not from the version count: each version's file count is declared as its share of the workload, an import books rows as they land, and the line reads "1,250,000 / 5,000,000 entries (25%)" beside the version fraction. Versions differ in rows by orders of magnitude, so a remaining time extrapolated from versions said nothing. The item line names the version under import with its dates, "Version 11 — 2026/9/4 10:59:10 → 11:01:00", on the browser's clock like the check and restore lists: the backend sends the two UTC timestamps in a fixed shape and the UI formats them |
| Checking catalog | catalog | the `PRAGMA quick_check` of the container's catalog file, a full read of it, owed only when a reader saw damage on it (see [storage-format.md](storage-format.md); an unclean exit used to count too, and cost 37 minutes on an 8 GB catalog after every `docker stop` that landed mid-import, so since 2026-09-13 it is logged and left to SQLite's journal). It used to run unannounced inside the first write open — minutes of "Loading versions 0%" on an 8 GB history, read as a hang — and now has a line of its own, **before** Loading versions (the load's own write open is where it used to hide), naming the file and its size ("catalog.db (8.2 GB)"). The check runs the same stage at its start, every time (the check is where an operator asks on purpose; see [check-restore-repair.md](check-restore-repair.md), "The local catalog"). It carries **no figures**: quick_check reports no progress, so there is no total, no percentage, no in-flight item and no speed — a 0% and a 0 B/s that stand for minutes read as a hang, where the file's size reads as the read it is. The counts line says "one full read of the catalog file". Pause is greyed as during Loading versions (there is nothing to park in); Suspend and Stop interrupt the statement and end the run, and the check simply runs again next time |
...
| Updating catalog | entries | an entry of the new version's index, read back off the file that went to the cloud and merged into the container's local catalog. The stage is the commit of the version — the info file, the catalog import, the journal, named in turn on the item line ("version 12 → catalog.db (8.2 GB)") — and it is named for the import because that is the step with something to count: the entries go past one at a time, merged against what the catalog already holds, and only the changes are written ([storage-format.md](storage-format.md)). It used to write **every** entry of the new version, which on a history of gigabytes was minutes of the stage standing at 100% while three content indexes re-sorted; format 2 (2026-09-13) retired both the writes and the wait. It used to be called Finalizing and carried no figures, so it borrowed the upload's — "Finalizing 100%" for ten minutes, a finish that would not finish. The counts line reads "recording the new version"; completion and remaining time come from the entries, as in Loading versions |
