...
| Loading versions | versions | a retained version made sure of in the catalog before the diff. Nothing on an ordinary run; on the first run after an upgrade it is the migration of the container's whole history, which can take a long time — and for as long as it had no stage of its own it sat under Scanning, where a migration taking hours read as a scan that had hung. Its completion and remaining time come from **entries** (index rows), not from the version count: each version's file count is declared as its share of the workload, an import books rows as they land, and the line reads "1,250,000 / 5,000,000 entries (25%)" beside the version fraction. Versions differ in rows by orders of magnitude, so a remaining time extrapolated from versions said nothing |
