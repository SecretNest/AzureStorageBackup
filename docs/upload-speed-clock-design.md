...
- **闸门空着时不报。** `VolumeUploadGate.AcquireAsync` 返回一个**已完成**的 Task 就表示一次队都没排，此时不标记——标记它等于给每一卷平白加一次强制发布，一件大活上千卷就是上千次。只有真排上队才报。另补一句 `ct.ThrowIfCancellationRequested()`：闸门空着时那条路一次都不看令牌，不补的话已经取消的运行会照常把这一卷传完才发现自己该停了。

> **实现更新（2026-08-08）：额度不再先到先得。**
>
> 闸门（`VolumeUploadGate`）按**件龄**仲裁：排序键是 `(票号, 卷号)`，一族卷在开始上传时领一张票，
> 票号小的优先，同族内按卷号升序。原先那个裸 `SemaphoreSlim` 是先到先得的，而消费者有
> `UploadConcurrency + 1` 个、压缩又是全局串行，于是稳态是「1 件在压 + N 件在传」，N 件各分到
> 大约一条流——**N 件同时半完成**。
>
> 这不只是难看。整族卷传完、云端确认返回之后才记 journal、才销在途账（`BackupOrchestrator`
> 的 `TrackInFlight`/`ClearInFlight`），所以「同时半完成的件数」就是一次中断白扔掉多少活：
> `Stop now` 要把在途件的残卷全删，挂起/崩溃则让它们整件重压重传。按件龄仲裁把这个数降到通常 1~2 件。
>
> 吞吐不受影响，额度始终满载：老件的窗口用不满时，空出来的额度当场落到下一件手上。
> 滑动窗口宽度因此取 `MaxParallelPerItem + 1`——多出来的那一卷是**接力棒**。少了它，
> 换卷那一瞬（`Release` 在 finally 里同步放行，而本族的下一卷要等续体才排得上队）额度会当场漏给新件，
> 每传完一卷漏一份，优先权名存实亡。
>
> 接力棒盖住的是常见时序，**不是绝对保证**：本族被放行之后要等续体才补上下一个等待者，这中间仍有
> 一道缝，线程池饿着时缝里可能漏走一卷。生产上缝以微秒计、一卷上传以秒计，所以至多是偶尔一卷；
> 要做到绝对得改成「一族卷各自攥着额度不放」，那是把这一层整个翻掉，与收益不成比例。
> 单测因此把界限定在「老族传完之前新族抢走的卷 < 2」（先到先得下稳定是一半），而不是 0。
