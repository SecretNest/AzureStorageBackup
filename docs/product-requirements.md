...
- **3.3 Local files**:
  ...
  - **3.3.2 Compression and encryption** (via 7z) — if the user selects neither compression nor encryption and does not use grouping, the original file is used directly; otherwise everything goes through a 7z wrapper.
    ...
    - **3.3.2.4 Staging area size** (1 GB default) — files to be backed up are compressed into this area for upload and deleted from it once uploaded. Compression must target a separate *compress-temp* directory first, and the result (possibly several volumes) is then moved here, so that no volume can be modified mid-compression. While the area is over its limit, no new file is compressed until it is under again; below the limit, the next compression task is dispatched. Compression tasks never run concurrently, not even across different backups. This design may let the area exceed its limit temporarily, by one newly added result.
      > **Implementation note (code is authoritative)**: the default is now **2 GB**, not 1 GB, and the limit is editable at runtime from Settings (`GlobalSettings.StagedLimitBytes`) rather than fixed at build time. The overshoot allowance also turned out to need widening: an uploader forced to recompress an item it must resend cannot queue for room — everything in the pool is released by an uploader, so one waiting for room waits for itself — so during a network outage the pool may exceed the limit by up to `UploadConcurrency + 1` archives. See [pipeline.md](pipeline.md).
...
- **3.4 Network concurrency** — upload/download concurrency, 5 by default.
  > **Implementation note (code is authoritative)**: the setting is global but the allowance is **per run**, not a shared pool. Every run builds its own gate from the stored value — `BackupOrchestrator` its uploaders, `RestoreOrchestrator` and `BackupChecker` their own `SemaphoreSlim` — and nothing is held across runs, so *n* concurrent operations open *n* × the configured number of connections. Concurrent operations on different targets are ordinary: the scheduler dispatches without awaiting and `BackupBusyTracker` only serialises per `account/container`. Contrast the staging area (3.3.x / `StagedLimitBytes`), which *is* one budget split across the runs holding a seat.
