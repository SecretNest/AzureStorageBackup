...
**Upload memory limit** (Settings → Performance) caps what those streams hold in RAM. Before a volume goes up it is hashed, and the cheapest way to make that hash describe exactly the bytes sent is to read the volume once into memory and upload from there — one volume per stream, so volume size × streams is the bill, and both factors are yours to raise. The limit (1 GB by default) is spent per operation like the concurrency: each backup, repair or compaction splits it evenly across its own upload streams. A volume that fits its stream's share is sent from memory; a bigger one is hashed from disk first and read a second time for the send — an extra read, never a missing label, so skipping unchanged volumes keeps working at any volume size. **0** means never hold a volume in memory; any non-zero limit still grants each stream at least 80 KB, one read chunk, because below that the memory route would hold less than the disk route's own buffer. The settings page shows the effective per-stream share against the default volume size, so you can see which route your volumes will take. **Encrypted backups do not spend this limit at all**: 7z encrypts every archive under a fresh random IV, so their volumes are different bytes on every run, a hash could never let a later run skip one, and none is computed — the volumes stream from disk through the upload's own 4 MB read buffer, unlabelled.
