...
/// <summary>
/// The boilerplate for a backup's local-authority wiring (version catalogs + local state) inside tests.
/// <para>
...
/// at every site: an in-memory SQLite, a <see cref="VersionCatalogs"/>, and a <see cref="TrackedInfoStore"/>.
...
internal sealed class TestLocalAuthority
{
    ...
    /// their own <see cref="VersionIndexFileStore"/> — which is the arrangement production has, and what lets a
    /// catalog migrate a version out of an <c>.idx</c> file some older build left there.</summary>
