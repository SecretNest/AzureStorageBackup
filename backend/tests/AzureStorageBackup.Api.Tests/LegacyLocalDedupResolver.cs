...
// A verbatim copy of LocalDedupResolver as it stood before its maps moved into the catalog and the run's work
// database, kept only as the oracle for LocalDedupResolverCatalogTests: the move to SQLite is meant to preserve
// behaviour, and the only way to say that with a straight face is to run both over the same generated versions and
// require every answer to agree. It is deliberately NOT kept in sync with the product class — it is the *old*
// behaviour, and freezing it is the whole point. It dies together with LocalDedupResolver.Build in Task 13.
...
/// </summary>
internal sealed class LegacyLocalDedupResolver
{
    ...
    private LegacyLocalDedupResolver(
    ...
    /// </para>
    /// </param>
    public static LegacyLocalDedupResolver Build(
        ...
        IEnumerable<ConfirmedBlob>? confirmed = null)
    {
        ...
        return new LegacyLocalDedupResolver(addressing, byContent, refs, heads, packMembers, damagedRefs);
    }
